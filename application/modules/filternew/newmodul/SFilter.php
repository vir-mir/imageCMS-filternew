...
class SFilter {
    ...
    public function __construct() {
        ...
        if ($_GET['lp'])
            $this->get_lp = (int) $_GET['lp'] / ShopCore::app()->SCurrencyHelper->getRateByfilter();
        if ($_GET['rp'])
            $this->get_rp = (int) $_GET['rp'] / ShopCore::app()->SCurrencyHelper->getRateByfilter();
    ...
    private function createFilterByBrandQueryString() {
        if (isset(ShopCore::$_GET['brand']) && is_array(ShopCore::$_GET['brand']) && get_class($this->model) == 'SCategory') {
            $brArr = "(";
    ...
    private function getProductsInBrandCount($brands = array()) {
        if (is_array($brands)) {
			...
			if (ShopCore::$_GET['lps'] && !empty(ShopCore::$_GET['lps']) && ShopCore::$_GET['rps'] && !empty(ShopCore::$_GET['rps'])) {
                foreach (ShopCore::$_GET['lps'] as $key => $value) {
                    $prFQ2 .= " AND `product_id` IN ";
                    $fq = (int)$value;
                    $fqMax = (int)$_GET['rps'][$key];
                    $propertyfilterQuery = "(SELECT `product_id` FROM `shop_product_properties_data` WHERE `locale`='" . $this->locale . "' AND `property_id`=" . $key . " AND `value` >= {$fq} AND `value` <= {$fqMax})";
                    $prFQ2 .= $propertyfilterQuery;
                }
            }
...
//                        exit();
    ...
    public function filterGet() {
        $allowedKeys = array('p', 'brand', 'lp', 'rp', 'lps', 'rps', 'order', 'per_page', 'user_per_page', 'category', 'utm_medium', 'utm_campaign', 'utm_source', 'gclid', 'filtermobile');
		...
		//properties filter variables escaping quotes
        if (isset(ShopCore::$_GET['lps'])) {
            foreach (ShopCore::$_GET['lps'] as $key => $item) {
                foreach ($item as $k => $v) {
                    ShopCore::$_GET['lps'][$key][$k] = ShopCore::encode(ShopCore::$_GET['lps'][$key][$k]);
                }
            }
        }
		//properties filter variables escaping quotes
        if (isset(ShopCore::$_GET['rps'])) {
            foreach (ShopCore::$_GET['rps'] as $key => $item) {
                foreach ($item as $k => $v) {
                    ShopCore::$_GET['rps'][$key][$k] = ShopCore::encode(ShopCore::$_GET['rps'][$key][$k]);
                }
            }
        }
