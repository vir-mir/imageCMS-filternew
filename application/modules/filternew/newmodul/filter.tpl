...
<aside class="span3">
    ...
    <div class="filter">
        ...
        {if count($show) != count($propertiesInCat)}
            ...
            <div class="boxFilter">
				{$modelPro = SPropertiesQuery::create()->findPk((int) $prop->property_id)}
				{$proVar = $modelPro->getVariableProperty()}
				{$multiple = $modelPro->getMultiple()}
                ...
                <div class="clearfix check_form">
					...
					{if $proVar == 'slide'}
						...
						<div class="sliderCont">
                        <div id="prop_{echo $prop->property_id}" class="ui-slider ui-slider-horizontal ui-widget ui-widget-content">
								<img src="{$THEME}images/slider.png" alt="slider"/>
								<div class="ui-slider-range ui-widget-header"></div>
								<a href="#" class="ui-slider-handle" style='margin-left: -8px;background-position: -316px 0;'></a>
								<a href="#" style='background-position: -326px 0;' class="ui-slider-handle" id="right_slider_{echo $prop->property_id}"></a>
							</div>
						</div>

						{//inputs for displaying price range in numeric format}
						<div class="formCost t-a_j">
						{$minV = reset($prop->possibleValues)}
						{$maxV = end($prop->possibleValues)}
							
							<label>
								{//left price value}
								<input type="text" name="" id="minV_{echo $prop->property_id}" value="{if ShopCore::$_GET['lps'][$prop->property_id] && (int)ShopCore::$_GET['lps'][$prop->property_id]>0 && (int)ShopCore::$_GET['lps'][$prop->property_id]>(int)$minV.value}{echo ShopCore::$_GET['lps'][$prop->property_id]}{else:}{echo (int)$minV.value}{/if}" data-title="только цифры" data-minS="{echo (int)$minV.value}"/> 
							</label>
							<span class="f-s_12">&ndash;</span>
							...
							<label>
								...
								{//rigth price value}
								<input type="text" name="" id="maxV_{echo $prop->property_id}" value="{if ShopCore::$_GET['rps'][$prop->property_id] && (int)ShopCore::$_GET['rps'][$prop->property_id]>0}{echo ShopCore::$_GET['rps'][$prop->property_id]}{else:}{echo (int)$maxV.value+1}{/if}" data-title="только цифры" data-maxS="{echo (int)$maxV.value+1}"/>
							</label>
						...
						<script>
							{literal}
								$(function () {							
									$('#prop_{/literal}{echo $prop->property_id}{literal}').slider({
										min: {/literal}{echo $minV.value}{literal},
										max: {/literal}{echo $maxV.value+1}{literal},
										range: true,
										values: [ 
											$('#minV_{/literal}{echo $prop->property_id}{literal}').val(), 
											$('#maxV_{/literal}{echo $prop->property_id}{literal}').val() 
										],
										slide: function( event, ui ) {
											if (
												ui.values[0] > {/literal}{echo $minV.value}{literal}
												|| ui.values[1] < {/literal}{echo $maxV.value+1}{literal}) {
												$('#minV_{/literal}{echo $prop->property_id}{literal}').attr('name', '{/literal}lps[{echo $prop->property_id}]{literal}')
												$('#maxV_{/literal}{echo $prop->property_id}{literal}').attr('name', '{/literal}rps[{echo $prop->property_id}]{literal}')
											} else {
											$('#minV_{/literal}{echo $prop->property_id}{literal}').attr('name', '')
												$('#maxV_{/literal}{echo $prop->property_id}{literal}').attr('name', '')
											}
											$('#minV_{/literal}{echo $prop->property_id}{literal}').val(ui.values[0])
											$('#maxV_{/literal}{echo $prop->property_id}{literal}').val(ui.values[1])											
										  }
									});
								})
							{/literal}
					...
					{if $proVar == 'list'}
                        ...
                        <div class="frameLabel">
							...
							<span class="propertySelect b_n">
								{//if we dont have products in current category with such possible value, we will disable checkbox}
								<select {if $multiple}multiple{/if} id="prop_{echo $prop->property_id}" class="propertyCheck" name="p[{echo $prop->property_id}][]">
					
							{foreach $prop->possibleValues as $item}
								
								<option {if is_array(ShopCore::$_GET['p'][$prop->property_id]) && in_array($item.value, ShopCore::$_GET['p'][$prop->property_id]) && $item.count != 0}selected="selected"{/if} value="{echo $item.value}">
								{echo $item.value}&nbsp;({//if $item.count != 0 && is_array(ShopCore::$_GET['p'][$prop->property_id]) && !in_array($item.value, ShopCore::$_GET['p'][$prop->property_id])}{///if}{echo $item.productCount})
								</option>
