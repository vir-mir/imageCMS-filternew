...
abstract class BaseSProperties extends ShopBaseObject  implements Persistent
{
	...
	/**
	 * Get the [variable_property] column value.
	 * 
	 * @return     boolean
	 */
	public function getVariableProperty()
	{
	...
	public function setVariableProperty($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->variable_property !== $v) {
			$this->variable_property = $v;
			$this->modifiedColumns[] = SPropertiesPeer::VARIABLE_PROPERTY;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->variable_property = ($row[$startcol + 10] !== null) ? $row[$startcol + 10] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(SPropertiesPeer::VARIABLE_PROPERTY)) {
			$modifiedColumns[':p' . $index++]  = '`VARIABLE_PROPERTY`';
		}
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`VARIABLE_PROPERTY`':						
						$stmt->bindValue($identifier, $this->variable_property, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 9:
				...
				break;
			case 10:
				return $this->getVariableProperty();
				break;
			default:
				return null;
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
	{
		...
		$result = array(
			...
			$keys[10] => $this->getVariableProperty(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 9:
				...
				break;
			case 10:
				$this->setVariableProperty($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[10], $arr)) $this->setVariableProperty($arr[$keys[10]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SPropertiesPeer::VARIABLE_PROPERTY)) $criteria->add(SPropertiesPeer::VARIABLE_PROPERTY, $this->variable_property);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setVariableProperty($this->getVariableProperty());
